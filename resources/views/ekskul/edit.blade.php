...
@section('content')
<h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Kegiatan Ekstrakurikuler</h1>

<form action="{{ route('ekskul.update', $ekskul->id) }}" method="POST" class="bg-white p-6 rounded shadow-md max-w-3xl">
    @csrf
    @method('PUT')
    ...
    <div class="mb-4">
        ...
        <textarea name="deskripsi" rows="4"
                  class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-red-500"
                  placeholder="Tulis deskripsi kegiatan">{{ old('deskripsi', $ekskul->deskripsi) }}</textarea>
    ...
    <div class="mb-4">
        ...
        <select name="pembina_id" required class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-red-500">
            @foreach($pembinas as $pembina)
                <option value="{{ $pembina->id }}" {{ old('pembina_id', $ekskul->pembina_id) == $pembina->id ? 'selected' : '' }}>
                    {{ $pembina->name }}
                </option>
            @endforeach
        </select>
    ...
    @else
        <input type="hidden" name="pembina_id" value="{{ $ekskul->pembina_id }}">
    @endif

    <!-- Jadwal -->
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700">Jadwal</label>
        <input type="text" name="jadwal" value="{{ old('jadwal', $ekskul->jadwal) }}"
               class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-red-500"
    ...
    <div class="mb-4">
        ...
        <input type="text" name="lokasi" value="{{ old('lokasi', $ekskul->lokasi) }}"
               class="w-full px-4 py-2 border rounded focus:ring-2 focus:ring-red-500"
